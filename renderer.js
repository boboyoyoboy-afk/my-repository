...
function renderTasks() {
  ...
  tasks.forEach((task) => {
    ...
    li.addEventListener('drop', (event) => {
      ...
      const sourceIndex = tasks.findIndex((item) => item.id === draggedTaskId);
      ...
      const adjustedTargetIndex = tasks.findIndex((item) => item.id === task.id);
      const insertIndex = dropAfter ? adjustedTargetIndex + 1 : adjustedTargetIndex;
